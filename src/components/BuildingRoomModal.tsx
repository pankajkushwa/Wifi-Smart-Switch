...
export const BuildingRoomModal: React.FC<BuildingRoomModalProps> = ({
  ...
  const activeBuilding = buildings.find(b => b.id === activeBuildingId) || (buildings.length > 0 ? buildings[0] : null);
  const activeBuildingRooms = activeBuilding ? rooms.filter(r => r.buildingId === activeBuilding.id) : [];
      ...
      >
        ...
        <div className="flex border-b border-slate-100 px-5 pt-3 bg-white">
          ...
          >
            ...
            <span>Rooms in {activeBuilding ? activeBuilding.name : 'None'} ({activeBuildingRooms.length})</span>
          </button>
        </div>
            ...
            <div className="space-y-4">
              {!isAddingBuilding ? (
                <>
                  ...
                  {/* Empty state if 0 buildings */}
                  {buildings.length === 0 && (
                    <div className="bg-slate-50 rounded-2xl p-6 text-center border border-dashed border-slate-200 space-y-2">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-400 flex items-center justify-center mx-auto">
                        <Building2 className="w-5 h-5" />
                      </div>
                      <h5 className="text-xs font-bold text-slate-700">No Homes or Buildings</h5>
                      <p className="text-[11px] text-slate-500 max-w-xs mx-auto">
                        You have no properties yet. Tap above to add a home or office to organize your rooms and switches.
                      </p>
                      <button
                        type="button"
                        onClick={() => setIsAddingBuilding(true)}
                        className="mt-1 px-3 py-1.5 bg-blue-600 text-white rounded-xl text-xs font-bold shadow-xs hover:bg-blue-500 inline-flex items-center space-x-1"
                      >
            ...
            <div className="space-y-4">
              {!activeBuilding ? (
                <div className="bg-slate-50 rounded-2xl p-6 text-center border border-dashed border-slate-200 space-y-2">
                  ...
                  <h5 className="text-xs font-bold text-slate-700">No Property Selected</h5>
                  <p className="text-[11px] text-slate-500 max-w-xs mx-auto">
                    Please add or select a property first in the Homes / Buildings tab before creating rooms.
                  </p>
                  <button
                    type="button"
                    onClick={() => {
                ...
                <>
                  ...
                  {/* Empty state if 0 rooms */}
                  {activeBuildingRooms.length === 0 && !isAddingRoom && (
                    <div className="bg-slate-50 rounded-2xl p-6 text-center border border-dashed border-slate-200 space-y-2">
                      <div className="w-10 h-10 rounded-xl bg-slate-100 text-slate-400 flex items-center justify-center mx-auto">
                        <Layers className="w-5 h-5" />
                      </div>
                      <h5 className="text-xs font-bold text-slate-700">No Rooms Added Yet</h5>
                      <p className="text-[11px] text-slate-500 max-w-xs mx-auto">
                        Add rooms like Living Room, Bedroom, Kitchen to organize your switches in {activeBuilding.name}.
                      </p>
                      <button
                        type="button"
                        onClick={() => setIsAddingRoom(true)}
        ...
        <div className="p-4 border-t border-slate-100 bg-slate-50 flex items-center justify-between">
          <div className="text-xs text-slate-500">
            Active: <strong className="text-slate-800">{activeBuilding ? activeBuilding.name : 'None'}</strong>
          </div>
          <button
            onClick={onClose}
